...
--- Explicaciones para ajustes /añadidos / cambios (gracias Duir)
----------------------------------------------------------------------------------------------------
1.  Añadido código de parche de seguridad "DROP TREE" de Jason Rohrer.
2.  Actualizada la pantalla de "acerca de" con la dirección sourceforge http://www.sourceforge.net/projects/mfc-mute-net
3.  La búsqueda de comodines "*" limita ahora el retorno de resultados. Los resultado son tratados aleatoriamente
    para incrementar la seguridad y reducir los ataques sobre búsquedas en red con comodines"
4.  Los archivos tales como "Desktop.ini" (Inicio de escritorio) y "Thumbs.db" (Iconos de imagen) no serán 
    incluidos nunca en los resultados de la búsqueda. No existe ninguna razón real para permitir la descarga
    de estos archivos, de forma que han sido eliminados de los resultados de la búsqueda.
5.  Los iconos con etiquetas dinámicas (dinamic tab icons) del arranque están en el directorio principal del programa.
    A continuación está la lista de estos archivos. Si algún usuario desea cambiar el icono que aparece en el arranque,
    simplemente ha de copiar el nuevo archivo de icono (.ico) en el directorio y darle el nombre que corresponda.
            icons\searchTab.ico
            icons\downloadsTab.ico (iconos\pestaña de descargas)
            icons\uploadsTab.ico  (iconos\pestaña de subidas)
            icons\connectionsTab.ico  (iconos\pestaña de conexiones)
            icons\sharedfilesTab.ico  (iconos\pestaña de archivos compartidos)
            icons\settingsTab.ico  (iconos\pestaña de configuración)
6.  Añadidas estadísticas de tráfico de red en varias pantallas (Actualmente sólo en Inglés).
7.  Añadido diálogo para configuración de puertos.
