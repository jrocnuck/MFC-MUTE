...
unsigned long OutboundChannel::getSentTime() {
    mLock->lock();
    unsigned long sentTime = mSentTime;
    mLock->unlock();
...
void OutboundChannel::run() {
    ...
    while( !stopped ) {
        ...
        if( message != NULL ) {
			...
			// it blocks here if the node is slow in receiving packets
            // the queue will continue to increase, bytesSent = -1
