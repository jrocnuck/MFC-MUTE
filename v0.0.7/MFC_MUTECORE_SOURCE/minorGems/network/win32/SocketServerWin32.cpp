...
SocketServer::SocketServer( int inPort, int inMaxQueuedConnections ) {
	...
	m_bSocketCatastrophicFailure = false;
	...
	if( !Socket::isFrameworkInitialized() ) {
		...
		if( error == -1 ) {
			...
			// jroc.. don't exit.. need to free up memory allocations
			m_bSocketCatastrophicFailure = true;
			//exit( 1 );
			return;
			}
		}
	...
	// jroc -- don't exit... because we will have memory leaks!
	if( error == -1  ) {
