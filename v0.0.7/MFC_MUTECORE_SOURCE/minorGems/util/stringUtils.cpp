 ...
 * jroc - 7/26/2004 changed to concatenate to fix spelling
 *
...
// jroc 7-26-2004 fixed spelling of function
char *concatenate( char *inStringA, char *inStringB ) {
...
char *autoSprintf( const char* inFormatString, ... ) {
    ...
    int bufferSize = 256; // jroc 01/20/05 -- made this longer.. why fail every time and have to go through all the hassle of deleting and reallocating so often?
	...
	// E. Nuckols 04-03-2004 .. VisualC++ uses _vsnprintf
#ifdef _WINDOWS
	int stringLength =
        _vsnprintf( buffer, bufferSize, inFormatString, argList );
#else
    int stringLength =
